...
#define CR 0x0D
#define CL 0x0A
#define Newline() UART_PutChar(CR); UART_PutChar(CL);
char uart_in;
char cadena_uart[100];
char MODO;
char enable_led='0';

CY_ISR(ISR_Timer1){ //timer de 2 segundos para apagar/prender la coneccion dependiendo del PP
    Timer1_ReadStatusRegister();
    if(enable_led=='1'){ // SI ESTA EN MODO ALARMA, EL LED SE PRENDE(1s).
        
    if(LED_Read()==1){ // SI ES QUE ESTA PRENDIDO SE APAGA
...
CY_ISR(IRQ_UART){//Validado, funciona y lee valores del 1 a 6.
...
void menu()
{
    ...
    UART_PutChar(0xA); UART_PutChar(0xD);
